...
export default function App() {
  ...
  const handleAiAnalysis = async () => {
    if (!result) return;
    setIsAiLoading(true);
    const analysis = await explainCalculation(result, renewalResult);
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            {!result ? (
              ...
              >
                ...
                {/* AI Analysis Section */}
                <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm no-print">
                  <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4">
                    ...
                    {!aiAnalysis && (
                      <button 
                        ...
                        className="rounded-lg bg-indigo-600 px-4 py-1.5 text-xs font-bold text-white shadow-md transition hover:bg-indigo-700 disabled:opacity-50"
                      >
                        {isAiLoading ? "Analisando..." : "Gerar Justificativa"}
                      </button>
                    )}
                  </div>
                  <div className="p-6">
                    {aiAnalysis ? (
                      ...
                      <p className="text-center text-sm italic text-slate-400">
                        Clique em "Gerar Justificativa" para obter uma análise técnica automática baseada na Lei 14.133.
                      </p>
                    )}
                  </div>
                </section>
